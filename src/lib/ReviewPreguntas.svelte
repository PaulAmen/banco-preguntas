<script>
  ...
  /** @type {{ preguntas: any[], onguardar: (p:any)=>Promise<boolean>, oncerrar: ()=>void }} */
...
<div class="review-overlay">
  <div class="review-card">
    <header class="review-header">
      ...
      <button class="btn-close" onclick={oncerrar} title="Cerrar">&times;</button>
    </header>
    ...
    {#if p}
      <div class="review-body">
        ...
        <div class="question-content">
          <p class="enunciado"><strong>{index + 1}. {p.Enunciado}</strong></p>

          {#if p.Tipo_Pregunta === 'Opción Múltiple' || p.Tipo_Pregunta === 'Estudio de Caso'}
            <ul class="opciones">
              {#if p.Opcion_A_o_Concepto1}<li><em>a)</em> {p.Opcion_A_o_Concepto1}</li>{/if}
              {#if p.Opcion_B_o_Definicion1}<li><em>b)</em> {p.Opcion_B_o_Definicion1}</li>{/if}
              {#if p.Opcion_C_o_Concepto2}<li><em>c)</em> {p.Opcion_C_o_Concepto2}</li>{/if}
              {#if p.Opcion_D_o_Definicion2}<li><em>d)</em> {p.Opcion_D_o_Definicion2}</li>{/if}
            </ul>
            <p><strong>Respuesta Correcta:</strong> {p.Respuesta_Correcta}</p>
          ...
          {:else if p.Tipo_Pregunta === 'Unir con Líneas'}
             <table class="tabla-unir">
                ...
                <tbody>
                  {#if p.Opcion_A_o_Concepto1 || p.Opcion_B_o_Definicion1}<tr><td>{p.Opcion_A_o_Concepto1}</td><td>{p.Opcion_B_o_Definicion1}</td></tr>{/if}
                  {#if p.Opcion_C_o_Concepto2 || p.Opcion_D_o_Definicion2}<tr><td>{p.Opcion_C_o_Concepto2}</td><td>{p.Opcion_D_o_Definicion2}</td></tr>{/if}
                  {#if p.Concepto3 || p.Definicion3}<tr><td>{p.Concepto3}</td><td>{p.Definicion3}</td></tr>{/if}
                  {#if p.Concepto4 || p.Definicion4}<tr><td>{p.Concepto4}</td><td>{p.Definicion4}</td></tr>{/if}
                </tbody>
             </table>
          {/if}
      ...
      <footer class="review-footer">
        <div class="nav-btns">
           <button class="btn-secondary btn-sm" onclick={anterior} disabled={index === 0} title="Pregunta anterior">
             ⬅️ Anterior
           </button>
        </div>
...
<style>
  ...
  .review-header h2 { margin: 0; font-size: 1.25rem; font-weight: 700; }
  .btn-close {
  ...
  .tabla-unir th, .tabla-unir td {
    border: 1px solid var(--borde);
    padding: 0.6rem;
    text-align: left;
  }
  .tabla-unir th { background: var(--azul-tenue); color: var(--azul-oscuro); }
  ...
  @media (max-width: 640px) {
    ...
    .review-footer { padding: 1rem; flex-direction: column; gap: 1rem; }
